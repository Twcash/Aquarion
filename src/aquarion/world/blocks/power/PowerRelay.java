...
public class PowerRelay extends PowerBlock {
    ...
    public static void getNodeLinks(Tile tile, Block block, Team team, Cons<Building> others){
        ...
        for(int i = 0; i < 4; i++){
            ...
            int fi = i;
            Building closest = tempBuilds.min(b -> b instanceof PowerRelayBuild node && node.couldConnect((fi + 2) % 4, block, tile.x, tile.y), b -> b.dst2(cx, cy));
            tempBuilds.clear();
            if(closest != null){
                others.get(closest);
            }
    ...
    public class PowerRelayBuild extends Building{
        ...
        public boolean couldConnect(int direction, Block target, int targetX, int targetY){
            ...
            AtomicBoolean connects = new AtomicBoolean(false);
        ...
        public void draw(){
            ...
            for(int i = 0; i < 4; i ++){
                if(dests[i] != null && links[i].wasVisible && (!(links[i].block instanceof PowerRelay node) ||
