...
public class AquaMenuDialog extends BaseDialog {
    ...
    private void createRoundAvatar(Table table, String textureName, Drawable fallback, float size) {
        table.image(getModIcon(textureName, fallback)).size(size).scaling(Scaling.fit);
    }
    ...
    private void updateContent(String type) {
        ...
        if (type.equals("links")) {
            ...
            bottomNav.button(Core.bundle.get("aquarion.menu.open_releases", "Open Releases on GitHub"), Icon.github, () -> Core.app.openURI(RELEASES_URL))
            ...
            for (Author author : Author.getHelpers()) {
                addAuthorButton(
                        inner, author.name, author.descKey, author.url,
                        author.texture, author.fallback, author.hasProfile,
                        buttonWidth, buttonHeight, avatarSize, padSize
                );
            }
    ...
    private void rebuildChangelogList(Seq<Jval> releases) {
        ...
        for (Jval release : releases) {
            ...
            changelogListTable.table(Styles.black5, t -> {
                ...
                t.table(actions -> {
                    actions.right();
                    float actionBtnHeight = 48f;

                    actions.button(Core.bundle.get("aquarion.menu.download_release", "Download"), Icon.download, () -> downloadAndInstall(finalDownloadUrl, finalTagName))
                            .width(160f).height(actionBtnHeight).padRight(8f);

                    actions.button(Core.bundle.get("aquarion.menu.open_release_tag", "View on GitHub"), Icon.export, () -> Core.app.openURI(htmlUrl))
    ...
    private void showAuthorInfo(String name, String description, String profileUrl, String textureName, Drawable fallbackIcon, boolean hasProfile) {
        ...
        if (hasProfile) {
            authorDialog.buttons.button(Core.bundle.get("aquarion.menu.open_profile"), () -> Core.app.openURI(profileUrl))
    ...
    public static void attach() {
        Events.on(ClientLoadEvent.class, e -> Vars.ui.menufrag.addButton(Core.bundle.get("aquarion.menu.button_main"), Icon.info, () -> new AquaMenuDialog().show()));
    }
}