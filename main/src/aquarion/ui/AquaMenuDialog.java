...
public class AquaMenuDialog extends BaseDialog {
    ...
    private void updateContent(String type) {
        ...
        // Размеры из предоставленного исходника
        ...
        if (type.equals("links")) {
            ...
            }).size(paneWidth, paneHeight - (Vars.mobile ? 90f : 70f)); // Запас высоты для нижней панели

            if (cell.get() instanceof ScrollPane) {
                ((ScrollPane)cell.get()).setFlickScroll(true);
            }

            // ВНЕ СКРОЛЛА: Пагинация и кнопка Open Releases зафиксированы внизу
            ...
            }).size(buttonWidth, 45f).padBottom(6f).row();

            Table paginationTable = new Table();
            float arrowSize = Vars.mobile ? 45f : 35f;
            ...
            paginationTable.label(() -> String.valueOf(page)).fontScale(1.1f).padLeft(15f).padRight(15f);
    ...
    private void rebuildChangelogList(Seq<Jval> releases) {
        ...
        float cardWidth = Vars.mobile ? 390f : 360f;
        ...
        for (Jval release : releases) {
            ...
            changelogListTable.table(Styles.black5, t -> {
                ...
                // Две переводимые кнопки
                t.table(actions -> {
                    actions.right();
                    
                    float actionBtnHeight = Vars.mobile ? 36f : 30f;
