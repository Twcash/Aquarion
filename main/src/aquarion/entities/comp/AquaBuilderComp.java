...
abstract class AquaBuilderComp implements Builderc, AquaBuilderc {
    @Annotations.Import
    float x, y, rotation, buildSpeedMultiplier;
    @Annotations.Import UnitType type;
    @Annotations.Import Team team;

    @Annotations.SyncLocal Queue<BuildPlan> plans = new Queue<>(1);
    @Annotations.SyncLocal boolean updateBuilding = true;
    ...
    public void updateBuildLogic(){
        ...
        float finalPlaceDst = Vars.state.rules.infiniteResources ? Float.MAX_VALUE : type.buildRange;
        boolean infinite = Vars.state.rules.infiniteResources || team().rules().infiniteResources;
        ...
        while((buildCounter >= 1 || instant) && count++ < maxPerFrame && plans.size > 0){
                ...
                if(!current.initialized && !current.breaking && Build.validPlaceIgnoreUnits(current.block, team, current.x, current.y, current.rotation, true, true)){
                    if(Build.checkNoUnitOverlap(current.block, current.x, current.y)){
                        boolean hasAll = infinite || current.isRotation(team) ||
                                //derelict repair
                                (tile.team() == Team.derelict && tile.block() == current.block && tile.build != null && tile.block().allowDerelictRepair && Vars.state.rules.derelictRepair) ||
                                //make sure there's at least 1 item of each type first
                                !Structs.contains(current.block.requirements, i -> !core.items.has(i.item, Math.min(Mathf.round(i.amount * Vars.state.rules.buildCostMultiplier), 1)));
            ...
            float bs = 1f / entity.buildCost * type.buildSpeed * buildSpeedMultiplier * Vars.state.rules.buildSpeed(team);

            //otherwise, update it.
            if(current.breaking){
                entity.deconstruct(self(), core, bs);
            }else if(entity.current != null && (Vars.state.isEditor() || (Vars.state.rules.waves && team == Vars.state.rules.waveTeam && entity.current.isVisible()) || (entity.current.unlockedNowHost() && entity.current.environmentBuildable() && entity.current.isPlaceable()))){ //only allow building unlocked blocks
    ...
    public void drawPlan(BuildPlan plan, float alpha){
        ...
        if(plan.breaking){
            ...
            Draw.color(Build.validPlace(plan.block, team, plan.x, plan.y, plan.rotation) || control.input.planMatches(plan) ? Color.gray.cpy(): Color.scarlet.cpy());
            Draw.rect(plan.block.fullIcon, plan.x, plan.y,plan.block.rotateDraw ? plan.rotation : 0);
    ...
    public boolean shouldSkip(BuildPlan plan, @Nullable Building core){
        if(state.rules.infiniteResources || team.rules().infiniteResources || plan.breaking || core == null || plan.isRotation(team)) return false;
