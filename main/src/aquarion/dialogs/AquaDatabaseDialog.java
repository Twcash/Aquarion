...
public class AquaDatabaseDialog extends BaseDialog {
    ...
    public AquaDatabaseDialog(){
        super("@database");
        ui.database.fill(t -> t.update(() -> {
                Vars.ui.database.hide(Actions.fadeOut(0f));
                ModEventHandler.databaseDialog.show();
        }));
        ...
        for(int j = 0; j < allContent.length; j++){
            ...
            all.table(list -> {
                ...
                for(var unlock : array){
                    ...
                    if(unlocked(unlock)){
                        String name = unlock.name;
                        int dash = name.indexOf('-');
                        if(dash != -1) name = name.substring(dash + 1);
                        String finalName = name;
                        image.clicked(() -> {
                            Log.info(finalName);
                            if(Core.input.keyDown(KeyCode.shiftLeft) && Fonts.getUnicode(finalName) != 0){


                                Core.app.setClipboardText((char)Fonts.getUnicode(finalName) + "");
                        ...
                        //Tooltip with prefix removed if shown
                        image.addListener(new Tooltip(t -> {
                            String displayName = unlock.localizedName;
                            String internalName = unlock.name;
                            int dashw = internalName.indexOf('-');
                            if(dashw != -1) internalName = internalName.substring(dashw + 1);

                            t.background(Tex.button).add(displayName + (settings.getBool("console") ? "\n[gray]" + internalName : ""));
                        }));
                    }
