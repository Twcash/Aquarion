...
public class AquaFx {
    ...
    hitCryoSmall = new Effect(14, e -> {
        ...
        randLenVectors(e.id, 2, 1f + e.fin() * 15f, e.rotation, 50f, (x, y) -> {
            ...
            factoryDestroyDefunct = new Effect(120, e -> {
                rand.setSeed(e.id);
                color(Color.valueOf("ffffff"), Color.valueOf("ffffff").a(e.fin()), e.fin());
                ...
                for (int i = 0; i < e.rotation * e.rotation*4+2; i++) {
                    ...
                    TextureRegion region = Core.atlas.find("aquarion-defunct-gore" + regionId);
                    v.trns(rot, rand.random(e.finpow() * 28f *e.rotation));
                    float fout = Math.max(e.fout(), 0.1f);
                    float size = fout * 14f;
                    Draw.rect(region, e.x + v.x, e.y + v.y, size, size, rand.random(270) * e.fout());
            ...
            infantryDeath = new Effect(20 * 60f, e -> {
                TextureRegion reg = Core.atlas.find("aquarion-infantry-corpse");
                float fadeStart = 0.8f; // 80% of lifetime passed before fade begins
                float alpha;
    ...
    concDeath = new Effect(20 * 60f, e -> {
        TextureRegion reg = Core.atlas.find("aquarion-concussor-corpse");
