...
public class FloraBlock extends Block {
    ...
    public void drawBase(Tile tile){
        if(emitLight)drawEnvironmentLight(tile);
        if( Mathf.chanceDelta(updateEffectChance * size) && !Vars.state.isPaused()) {
            effect.at(tile.worldx()+Mathf.range(size*4), tile.worldy()+Mathf.range(size*4));
        }
        float
