...
public class AssembleUnitBlock extends Block {
    public TextureRegion inactiveRegion;
    public Sound startsound;
    public ObjectMap<float[], Block> reqs = new ObjectMap<float[], Block>();
    public AssembleUnitBlock(String name) {
        super(name);
    ...
    public class UnitBlockBuild extends Building {
        ...
        public void draw() {
                reqs.forEach((requirement) ->{
                    Draw.z(Renderer.Layer.heat);

                    Tmp.v1.set(requirement.key[0]*8,requirement.key[1]*8);
                    Tmp.v1.rotate(rotation*90-90);
                    Draw.color(Pal.lightishGray);
                    Draw.alpha(0.5f);
                    Draw.rect(requirement.value.region,x+ Tmp.v1.x, y+Tmp.v1.y, 0);
                    Draw.reset();
                    /*
                    ________
                    ||||||||
        ...
        public void updateTile(){
            reqs.forEach((requirement) ->{
                ...
                if(tile.block()!= null){
                    if(tile.block() != requirement.value){
                        efficiency *= 0;
                    }
                }
            });
