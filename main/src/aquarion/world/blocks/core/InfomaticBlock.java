...
public class InfomaticBlock extends MessageBlock {
    ...
    public class infoBuild extends MessageBlock.MessageBuild{
        ...
        public void drawMessage(){
            ...
            String text;
            if(raw.startsWith("@") && !raw.startsWith("@Ceyf")){
                text = Core.bundle.get(raw.substring(1));
            }else{
                text = raw;
            }
            if(text.startsWith("@Ceyf")){
                String encrypted = Core.bundle.get(raw.substring(1));
                text = BundleCipher.decrypt(encrypted);
            }
...
class BundleCipher {
    ...
    private static final String KEY = "infomatic-key";

    public static String decrypt(String input){
        String out = Arrays.toString(Base64.decode(input));
        return out;
    }
...
//COPY PASTED FROM SUN.TOOLS.
//This is the deprecated vers, hopefully this aint that bad.
class  Base64 {
